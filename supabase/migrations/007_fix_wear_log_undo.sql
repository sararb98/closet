...
DROP TRIGGER IF EXISTS update_scheduled_outfit_wear_counts_trigger ON calendar_outfit_instances;

CREATE TRIGGER update_scheduled_outfit_wear_counts_update_trigger
    AFTER UPDATE OF status ON calendar_outfit_instances
    FOR EACH ROW
    EXECUTE FUNCTION update_scheduled_outfit_wear_counts();

CREATE TRIGGER update_scheduled_outfit_wear_counts_delete_trigger
    BEFORE DELETE ON calendar_outfit_instances
    FOR EACH ROW
    EXECUTE FUNCTION update_scheduled_outfit_wear_counts();